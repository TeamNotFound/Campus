...
public class DataAppelloDao implements DataAppelloInterfaces {
	...
	public void inserimento(DataAppello dataAppello) {
		...
		try {
			...
			System.out.println("Errore inserimento data appello dao");
			e.printStackTrace();
	...
	public DataAppello getById(int id) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Errore getById in data appello dao");
			session.getTransaction().rollback();
		}finally {
			session.close();
		}
	...
	public ArrayList<DataAppello> getAll() {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Errore getAll in data appello dao");
			session.getTransaction().rollback();
			return null;
		}finally {
			session.close();
	...
	public void update(DataAppello dataAppello) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Errore update in data appello dao");
			session.getTransaction().rollback();
		}finally {
			session.close();
		}
	...
	public void remove(DataAppello dataAppello) {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("Errore remove in data appello dao");
			session.getTransaction().rollback();
		}finally {
			session.close();
		}
